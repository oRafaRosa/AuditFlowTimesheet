  ...
  const renderMatrixSVG = (mode: 'default' | 'fullscreen' = 'default') => {
    ...
    >
      ...
      {records.filter(r => selectedCodes.has(r.code)).map((record) => {
        ...
        return (
          // O grupo inteiro se move junto (bolinha + numero) via transform animado
          <g
            key={record.id}
            style={{
              cursor: 'pointer',
            ...
            onMouseLeave={() => setHoveredRisk(null)}
          >
            <circle cx={0} cy={0} r={12} fill={pointColor} opacity={0.15} />
            <circle cx={0} cy={0} r={9} fill={pointColor} stroke="#ffffff" strokeWidth={1.5} />
            <text x={0} y={3} textAnchor="middle" fontSize={8} fontWeight="700" fill="#ffffff">{formatRiskCodeForDisplay(record.code)}</text>
          </g>
        );
      })}
